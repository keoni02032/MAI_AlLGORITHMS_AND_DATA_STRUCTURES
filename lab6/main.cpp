...
const long DEFAULT_BASE_FOR_LONG_NUMBER = 1000000;

class TLongNumber {
private:
	long base = DEFAULT_BASE_FOR_LONG_NUMBER;
...
TLongNumber::TLongNumber(long a) {
	if (a == 0) {
...
TLongNumber Sum(const TLongNumber& a, const TLongNumber& b) {
	long transfer = 0;
	long size = std::max(a.container.size(), b.container.size());
	long s = 0;
	TLongNumber C;
...
TLongNumber Mult(const TLongNumber& a, const TLongNumber& b) {
	...
	for (size_t i = 0; i < a.container.size(); ++i)
		for (int j = 0, carry = 0; j < (int)b.container.size() || carry; ++j) {
			long cur = c.container[i + j] + a.container[i] * 1ll * (j < (int)b.container.size() ? b.container[j] : 0) + carry;
...
TLongNumber SmallDiv(const TLongNumber& a, const int& b) {
	...
	int carry = 0;
	for (int i = (int)a.container.size() - 1; i >= 0; --i) {
		long cur = a.container[i] + carry * 1ll * a.base;
	...
	while (std::cin >> str1 >> str2 >> Operator) {
		...
		case '^':
			if (a == 0 && b == 0) {
				...
				std::cout << SmallExp(a, b) << std::endl;
				std::cin.get();
			}
			break;
		case '/':
