...
class SuffTree {
   public:
    SuffTree(const std::string &s) : str(std::move(s + SENTINEL)) {
...
std::vector<int> SuffTree::GetMatchStatistic(const std::string &text) {
...
int main()
{
	...
	SuffTree Tree(pattern);

	std::string text;
	std::cin >> text;

	std::vector<int> res = Tree.GetMatchStatistic(text);

	size_t patLength = pattern.length();
